...
class ManageApiController extends Controller {
    ...
    {
        $data['users'] = DB::table('lp_users')->where('user_id', $user_id)
                        ->select('user_id', 'wallet_address', 'status', 'created_at')->get();
    ...
    public function getInsertCurrencyData($curr_id){

        $data['get_currency_data'] = DB::table('lp_currency')
                                ->where('id', '=', $curr_id)
                                ->select('id', 'name', 'currency_code', DB::raw('(CASE WHEN image = "" THEN "" ELSE CONCAT ("' . $this->base_url . '/uploads/currency_image/", image) END) AS image'), 'contract_address', 'abi_key', 'decimal_point', 'private_key', 'receive_wallet_address')
                                ->get();
    ...
    public function updateCurrencyData(Request $request, $curr_id)
    {
        ...
        if($file == ''){
            ...
            $allowed_type_arr = array('jpeg', 'png','jpg', 'svg');
